...
export class Zip implements Tool {
    getName(): string {
        return "zip";
    }
    async getOutput(uri: vscode.Uri): Promise<string> {
        ...
        try {
            ...
            if (exception.signal === undefined) {
                const error_msg = `Cannot run 'zip' via command '${executable}'. Please install it or set the option 'vscode-linux-binary-preview.zip_command'.`;
